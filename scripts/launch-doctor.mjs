...
async function main() {
  ...
  let shareMemory = null;
  if (supabaseUrl) {
    shareMemory = await checkShareMemoryPreview({ supabaseUrl, publishableKey, publicSiteUrl });
  ...
  statusLine(
    ...
    'Edge Function Secret PUBLIC_SITE_URL',
    publicSiteUrl
      ? shareMemory?.hasPublicSiteRedirect
        ? `远端 share-memory 已输出主站回跳：${shareMemory.publicSiteRedirectUrl}`
        : '远端 share-memory 未输出与 VITE_PUBLIC_SITE_URL 匹配的主站回跳'
      : '缺少 VITE_PUBLIC_SITE_URL，无法比对远端函数回跳',
  );
  ...
  if (!publicSiteUrl || !shareMemory?.hasPublicSiteRedirect) {
