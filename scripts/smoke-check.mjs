...
async function verifyLocalSite() {
  ...
  try {
    ...
    await page.getByLabel('当前画廊范围').waitFor({ state: 'visible' });
    ...
    await page.getByPlaceholder('按标题、描述、标签或上传者查找').fill(firstCardTitle);
    await page.getByLabel('当前画廊范围').getByText(`搜索：${firstCardTitle}`).waitFor({ state: 'visible' });
    ...
    await page.getByPlaceholder('按标题、描述、标签或上传者查找').fill('');
    ...
    const selectedTagValue = await page.getByLabel('标签筛选').inputValue();
    if (selectedTagValue !== firstViewerTag) {
      fail(`大图标签筛选没有同步到画廊筛选器：期望 ${firstViewerTag}，实际 ${selectedTagValue || '空'}`);
    }
    await page.getByLabel('当前画廊范围').getByText(`标签：${firstViewerTag}`).waitFor({ state: 'visible' });
...
async function verifyMobileSite() {
  ...
  try {
    ...
    const memoryButtons = page.locator('.memory-open');
    if ((await memoryButtons.count()) < 1) {
...
async function verifyAuthenticatedSite({
  ...
  try {
    ...
    if (session) {
      await page.getByRole('button', { name: '我的图片' }).waitFor({ state: 'visible' });
    } else {
    ...
    if (expectAdmin) {
      ...
      await adminPage.getByRole('button', { name: '用户管理' }).click();
      await adminPage.getByText('邀请用户').waitFor({ state: 'visible' });
      await adminPage.getByRole('button', { name: '站点设置' }).click();
      ...
      await page.getByRole('heading', { name: '记忆碎片' }).waitFor({ state: 'visible' });
      await page.locator('.gallery-scope-bar .scope-pill.active').filter({ hasText: '我的图片' }).waitFor({ state: 'visible' });
