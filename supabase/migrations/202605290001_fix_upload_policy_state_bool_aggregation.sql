-- Fix upload policy aggregation for Postgres, which does not support max(boolean).
...
begin
  ...
  with setting_rows as (
    select
      bool_or((value #>> '{}')::boolean) filter (where key = 'uploads_enabled') as uploads_enabled_value,
      max((value #>> '{}')::integer) filter (where key = 'upload_hour_limit') as upload_hour_limit_value,
      max((value #>> '{}')::integer) filter (where key = 'upload_day_limit') as upload_day_limit_value
    from public.site_settings
    where key in ('uploads_enabled', 'upload_hour_limit', 'upload_day_limit')
  ),
  profile_row as (
    select
      user_profiles.can_upload,
      user_profiles.upload_limit_total
